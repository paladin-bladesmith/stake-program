...
use crate::{
    ...
    require,
    state::{
        calculate_eligible_rewards, calculate_stake_rewards_per_token, find_sol_staker_stake_pda,
        find_validator_stake_pda, Config, SolStakerStake, ValidatorStake, EMPTY_RETURN_DATA,
    },
};
...
pub fn process_harvest_sync_rewards(
    ...
    if stake_amount != sol_staker_stake.lamports_amount {
        ...
        //
        //   1. Calculate the rewards earned by the SOL staker stake account, so we
        //      can determine how much fees the searcher should receive. Searcher fee
        //      is the min of (rewards available, config search fee)
        //
        //   2. Update the `last_seen_stake_rewards_per_token` on the SOL staker stake
        //      account to deduct the searcher fee. This will decrease the rewards per
        //      token for the current SOL staker stake account to take into consideration
        //      the searcher reward.
        //
        //   3. Transfer the searcher fee to the destination account
        //
        //   4. Update the SOL staker and validator stake accounts' lamports amount
        //
        ...
        // sync SOL stake lamports amount on the validator and SOL staker stake accounts
