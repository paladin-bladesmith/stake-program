...
#[tokio::test]
async fn fail_initialize_config_with_missing_token_extensions() {
    let mut context = ProgramTest::new("stake_program", paladin_stake::ID, None)
        .start_with_context()
        .await;
    ...
    // And a token account with a missing extension.
    ...
    create_token_account(
        ...
        &token,
        &mint.pubkey(),
        &[ExtensionType::TransferHookAccount], // <- missing ImmutableOwner extension
    )
    .await
    .unwrap();

    // When we try to initialize the config with a token account missing an extension.
    ...
    assert_custom_error!(err, StakeError::MissingTokenAccountExtensions);
}
