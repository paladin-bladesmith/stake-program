...
import { AccountType, getAccountTypeEncoder } from '../types';
...
export function identifyStakeAccount(
  ...
  if (
    containsBytes(data, getAccountTypeEncoder().encode(AccountType.Config), 0)
  ) {
    return StakeAccount.Config;
  }
  throw new Error(
    'The provided account could not be identified as a stake account.'
  );
}
